...
import static invoker54.xpshop.event.generation.ShopGenerationCopyEvent.assignCategory;

@Mod.EventBusSubscriber(modid = XPShop.MOD_ID)
public class CraftRecipeCopyEvents {

    private static final ModLogger LOGGER = ModLogger.getLogger(XPShopConfig.debugMode);

    @SubscribeEvent
    public static void priceRecipe(PriceRecipeCopyEvent event) {
...
//        LOGGER.info("Pricing craft-able(?) item: " + itemName);
        ...
        itemData.getPriceList(itemData, new ArrayList<>(), new ArrayList<>());
    }

    @SubscribeEvent
    public static void onBrew(PriceRecipeCopyEvent event) {
        ItemStack currentItem = event.getCurrentItem();
...
//        LOGGER.info("Item class: " + currentItem.getItem().getClass());
        ...
        List<PriceGroup> groupList = RecipeData.createFakeGroup(ingredientList);
        ...
        PriceList.PriceInfo finalResult = new PriceGroup(null, groupList, op, null).getResult();

        //If the potion isn't craftable, skip it.
        if (!hasIngredients.get()) return;
