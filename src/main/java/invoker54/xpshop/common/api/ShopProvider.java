...
public class ShopProvider implements ICapabilitySerializable<INBT> {
    public static final byte COMPOUND_NBT_ID = new CompoundNBT().getId();

    public ShopProvider(World level){
        shopCapability = new ShopCapability(level);
    }

    //region Capability setup
    //This is where all of the fallen capability data is
    @CapabilityInject(ShopCapability.class)
    public static Capability<ShopCapability> XPSHOPDATA = null;

    private final static String XPSHOP_NBT = "xpShopData";
    ...
    public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> capability, @Nullable Direction facing) {
        ...
        if (XPSHOPDATA == capability) {
            return LazyOptional.of(() -> shopCapability).cast();
    ...
    public INBT serializeNBT() {
        ...
        INBT shopNBT = XPSHOPDATA.writeNBT(shopCapability, null);
    ...
    public void deserializeNBT(INBT nbt) {
        ...
        XPSHOPDATA.readNBT(shopCapability, null, nbtData.getCompound(XPSHOP_NBT));
    }
